...
依赖分析器（dependency_analyzer）模块是一个全面的多语言代码分析框架，旨在提取、分析和可视化代码仓库中的依赖关系。它提供了用于理解代码结构、函数调用、类关系以及软件项目整体架构的工具。

## 概述

该模块支持对多种编程语言进行解析和分析，包括 Python、JavaScript、TypeScript、Java、C#、C、C++、PHP 和 Go。它提取代码组件（函数、类、方法、接口等）并构建依赖关系图，帮助开发者理解代码库中不同部分之间的关联。
...
| 组件 | 文件 | 描述 |
|-----------|------|-------------|
| DependencyParser | [dependency_analyzer.md#dependencyparser](dependency_analyzer.md#dependencyparser) | 从多语言仓库中提取代码组件 |
| DependencyGraphBuilder | [dependency_analyzer.md#dependencygraphbuilder](dependency_analyzer.md#dependencygraphbuilder) | 构建和管理依赖关系图，支持拓扑排序 |

### 分析服务

| 组件 | 文件 | 描述 |
|-----------|------|-------------|
| AnalysisService | [analysis_service.md](analysis_service.md) | 仓库分析的集中编排器 |
| CallGraphAnalyzer | [call_graph_analyzer.md](call_graph_analyzer.md) | 多语言调用图分析协调器 |
| RepoAnalyzer | [repo_analyzer.md](repo_analyzer.md) | 仓库结构和文件树分析 |

### 语言专用分析器

该模块包含针对每种受支持编程语言的专用分析器：
...
| 模型 | 文件 | 描述 |
|-------|------|-------------|
| Node | [models.md](models.md) | 表示代码组件（函数、类、方法） |
| CallRelationship | [models.md](models.md) | 表示组件之间的依赖关系 |
| Repository | [models.md](models.md) | 包含仓库元数据 |
| AnalysisResult | [analysis_models.md](analysis_models.md) | 包含可视化数据的完整分析输出 |

### 工具

| 组件 | 文件 | 描述 |
|-----------|------|-------------|
| ColoredFormatter | [logging_config.md](logging_config.md) | 带颜色的控制台日志，便于调试 |
...
dependency_analyzer 模块与 CodeWiki 系统中的其他几个模块集成：

- **cli_config** - 使用 Config 进行设置管理
- **be_agent_orchestrator** - 为文档生成提供分析结果
- **cli_adapters** - 与 DocTranslator 配合进行多语言文档生成
- **core_utils** - 使用 FileManager 进行文件操作

另请参阅：[cli_adapters.md](cli_adapters.md)、[cli_config.md](cli_config.md)、[be_agent_orchestrator.md](be_agent_orchestrator.md)、[core_utils.md](core_utils.md)
